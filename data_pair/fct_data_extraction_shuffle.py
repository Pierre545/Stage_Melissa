...
class little_pair(raster_data):
    ...
    def crop_pair_hv(self, path2tensor_1, path2tensor_2):
        ...
        while len(self.r_array)>0 :
            i = i+1
            # print(f"size of r_data {len(self.r_array)} valeur de i {i}")

            tmp_1 = self.r_array[index]
            tmp_2 = self.c_array[index]
            ...
            if percentage_zero_crop1 >= 99 and percentage_zero_crop2 >= 99 and percentage_nine_crop2 <= 1 and percentage_inf <= 1:
                ...
                if self.overlap is not None:
                    ...
                    a = self.r_array[index] + self.batch_size * self.percentage_overlap
                    b = self.c_array[index] + self.batch_size * self.percentage_overlap
                    little_pair.no_overlap(self, a, b)
                    print(f"Size of r_array {len(self.r_array)}")
                    #Construct a new index_list with the new size of the list
                    index_list = np.arange(0, len(self.r_array), 1)


                    #Avoid selecting each time the same index
                    if not index in used and len(self.r_array) :
                        used.append(index)
                        index = random.choice(index_list)
                    elif len(self.r_array) :
                        while index in used:
                            index = random.choice(index_list)
            ...
            #else:
                #remove index of value unaccepted
...
class images_pair() :
    ...
    def write_dict(dict_pair,out_path):
        #dict_pair should be the dictionnary containing the pair of path associated with file
        try:
